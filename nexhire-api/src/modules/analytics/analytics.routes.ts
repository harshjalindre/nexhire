...
export async function analyticsRoutes(fastify: FastifyInstance) {
  ...
  // #9 — XSS-safe placement report (HTML — can be printed to PDF via browser)
  ...
  fastify.get("/report/placement", async (req, reply) => {
    ...
    const html = `<!DOCTYPE html><html><head><title>Placement Report — ${esc(tenant?.name)}</title><style>body{font-family:sans-serif;max-width:800px;margin:0 auto;padding:20px}table{width:100%;border-collapse:collapse;margin:16px 0}th,td{border:1px solid #ddd;padding:8px;text-align:left}th{background:#f3f4f6}.header{text-align:center;border-bottom:2px solid #6366f1;padding-bottom:16px;margin-bottom:24px}.stat{display:inline-block;margin:0 20px;text-align:center}.stat-num{font-size:28px;font-weight:bold;color:#6366f1}.stat-label{font-size:12px;color:#666}@media print{body{padding:0}}</style></head><body>
      ...
      <h3>Drives Summary</h3><table><tr><th>Drive</th><th>Company</th><th>Package</th><th>Applications</th><th>Status</th></tr>${drives.map(d => `<tr><td>${esc(d.title)}</td><td>${esc(d.company.name)}</td><td>₹${d.packageLpa} LPA</td><td>${d._count.applications}</td><td>${esc(d.status)}</td></tr>`).join("")}</table>
      <h3>Student Placements</h3><table><tr><th>Name</th><th>Branch</th><th>CGPA</th><th>Status</th></tr>${students.slice(0, 50).map(s => `<tr><td>${esc(s.user.name)}</td><td>${esc(s.branch)}</td><td>${s.cgpa}</td><td>${esc(s.placementStatus)}</td></tr>`).join("")}</table>
